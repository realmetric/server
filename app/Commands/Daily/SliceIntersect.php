...
//        }
    ...
    private function process()
    {
        ...
        $this->fillFakeData($minute);
//
//        $uniqueCombs = $this->mysql->dailyRawAllSlices->getAllByMinute($minute);
//
//        foreach ($uniqueCombs as $comb) {
//
...
//        }
    ...
    private function fillFakeData($minute)
    {
        $metrics = $this->mysql->dailyMetrics->getMetricsByMinute($minute);
        foreach ($metrics as $metricId) {
            $slicesAll = $this->mysql->dailySlices->getSlicesByMetric($metricId, $minute);
            $slicesAll = array_slice($slicesAll, 0, 20);
            $value = mt_rand(100, 10000);
            $this->mysql->dailyRawAllSlices->create($metricId, $slicesAll, $value, $minute);
        }
    }
}