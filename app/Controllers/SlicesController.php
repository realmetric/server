...
class SlicesController extends AbstractController
{
    public function getByMetricId(ServerRequestInterface $request)
    {
        ...
        if (empty($attributes['metric_id'])) {
            throw new \InvalidArgumentException('Invalid metric_id(' . $attributes['metric_id'] . ')');
        }

        $values = $this->getSliceValues($attributes['metric_id'], $from, $to);
    ...
    protected function getFormattedTotalsFromMonthlySlices(int $metricId, \DateTime $from, \DateTime $to, $periodDiffDays): array
    {
